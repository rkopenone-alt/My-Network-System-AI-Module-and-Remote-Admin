# 📘 Rescue Operations System (ARDMS) — Localhost Deployment & Troubleshooting Guide (Version 2.0)
...
* **AI Auto-Assignment Module:** An automated background process powered by **Google Gemini 1.5 Pro AI**. It intelligently processes live situational data, interpreting JSON telemetry payloads to assign new incoming SOS requests to the most appropriate rescuer. The AI evaluates configurable parameters like real-time GPS proximity, rescuer workload, traffic estimations, and unit type suitability.
* **Reassignment Buffer System:** Implements a dynamic fail-safe timeout logic. If an assigned rescuer goes offline, ignores the task beyond the configured buffer interval, or explicitly declines the mission, the system instantly revokes the assignment and autonomously triggers the AI to re-evaluate and route the task to the next optimal candidate.
...
3. **Configure Windows Firewall:** Open PowerShell as **Administrator** (Right-click PowerShell -> Run as Administrator) and run:
   ...
   ```
4. **Synchronize Endpoints & Inject Server IP:** Open CMD, navigate to the workspace, and run the synchronizer to bind the apps to your specific local network:
   ```cmd
   python sync_apps.py 192.168.X.X  # Replace with your actual LAN IP
   ```
5. **Compile Standalone APKs:** After synchronizing the IP addresses, you MUST recompile the individual React Native WebViews into installable Android APKs using the provided Python build scripts:
   ```cmd
   python build_public_apk.py  # Builds the Citizen SOS App
   python build_rescuer.py     # Builds the Rescue Officer Field App
   python build_admin_apk.py   # Builds the Mobile Command Admin App
   ```
6. **Start Server:** Navigate to `system-backend` and run `npm start`.
7. **Install APKs:** Copy the generated `.apk` files from the root directory or `Output_APKs/` to your Android devices and install them.
...
* **Step 5: Synchronize and Start the Rescue Platform:**
  ...
  * Run the synchronizer script and pass your specific NIB core TUN IP as an argument (e.g., `10.45.0.1`) to override dynamic LAN detection and configure the API routes to bind to the virtual cellular base station gateway:
    ...
    ```
  * Compile the APKs using the provided build scripts so the cellular network endpoints are hardcoded into the apps:
    ```cmd

...
// ΓöÇΓöÇΓöÇ Color Palette ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const C = {
...
const taskIcon = (type) =>
  type === 'sos' ? '≡ƒÜ¿' : type === 'medical' ? '≡ƒÅÑ' : type === 'pregnancy' ? '≡ƒÜæ' : '≡ƒì▒';
    ...
    <SafeAreaView style={s.loginContainer}>
      ...
      <Animated.View style={[s.loginCard, { transform: [{ translateX: shake }] }]}>
        <View style={s.logoCircle}>
          <Text style={{ fontSize: 40 }}>≡ƒÜü</Text>
        </View>
...
// ΓöÇΓöÇΓöÇ Task Card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  return (
    <View style={[
      ...
      <View style={s.taskBody}>
        <Text style={s.taskLocation}>≡ƒôì {task.sector || 'Coordinate Point'}</Text>
        {task.message && (
          <Text style={s.taskMessage}>"{task.message}"</Text>
        )}
        <View style={s.taskMetaRow}>
          <Text style={s.metaText}>ΓÅ▒∩╕Å {dist} KM</Text>
          <Text style={s.metaText}>≡ƒåö {task.id.split('_')[1]}</Text>
        </View>
      </View>
...
// ΓöÇΓöÇΓöÇ Dashboard Screen ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
function DashboardScreen({ user, onLogout }) {
  ...
  const doSync = async () => {
    try {
      ...
      if (data.notifications?.length > 0) {
        ...
        showNotif(`≡ƒôó HQ: ${firstNotif.message}`, firstNotif.action_required === 1);
      }
      ...
      // Handle incoming commands ΓÇö detect critical ones for popup
      if (data.commands?.length > 0) {
        const critCmd = data.commands.find(c => c.command_type === 'critical' && (c.status === 'pending' || c.status === 'assigned'));
        if (critCmd) {
        ...
        setTasks(prev => {
          ...
          const newCmds = data.commands
            .filter(c => !cmdIds.has(c.id) && (c.status === 'pending' || c.status === 'assigned'))
            .map(c => {
  ...
  const declineTask = async (taskId) => {
    ...
    showNotif('Task declined ΓÇö returned to command queue.', false);
  };
  ...
  const completeTask = async (taskId) => {
    ...
    setTasks(prev => prev.filter(t => t.id !== taskId));
    showNotif('Γ£à Mission Completed successfully', false);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.dark }}>
        ...
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 9999, justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <Animated.View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 28, width: '100%', borderTopWidth: 6, borderTopColor: C.danger, shadowColor: '#000', shadowOpacity: 0.5, shadowRadius: 20, elevation: 20 }}>
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
              <Text style={{ fontSize: 48, marginBottom: 8 }}>≡ƒÜ¿</Text>
            ...
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  showNotif('Critical task declined ΓÇö returned to command queue.', false);
                }}>
                <Text style={{ color: C.danger, fontWeight: '900', fontSize: 16 }}>Γ£ù DECLINE</Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                onPress={async () => {
                  ...
                  setCriticalAlert(null);
                  showNotif('≡ƒÜ¿ Critical task accepted ΓÇö proceed immediately!', true);
                  // Trigger acceptance logic (focus map etc)
                  acceptTask(`cmd_${criticalAlert.cmdId}`);
                }}>
                <Text style={{ color: '#fff', fontWeight: '900', fontSize: 16 }}>Γ£ô ACCEPT</Text>
              </TouchableOpacity>
            </View>
          </Animated.View>
        </View>
        ...
        {tab === 'tasks' ? (
          <>
              ...
              {['sos', 'medical', 'food'].map(type => (
                ...
                >
                  <Text style={[s.filterBtnText, filters[type] && s.filterBtnTextActive(type)]}>
                    {type === 'sos' ? '≡ƒÜ¿ SOS' : type === 'medical' ? '≡ƒÅÑ Medical' : '≡ƒì▒ Food'}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>

            {/* ≡ƒôì Map View Integration */}
              ...
              >
                ...
                <Marker coordinate={{ latitude: 13.0700, longitude: 80.2600 }}>
                  <View style={s.selfMarker}>
                    <Text style={{ fontSize: 16 }}>≡ƒÜü</Text>
                  </View>
                </Marker>
            ...
            <View style={s.countBanner}>
              <Text style={s.countText}>
                {activeTask ? '≡ƒôì Active Mission' : `${visibleTasks.length} Task${visibleTasks.length !== 1 ? 's' : ''} Available`}
              </Text>
              {!activeTask && (
                <TouchableOpacity onPress={fetchTasks}>
                  <Text style={{ color: C.primary, fontSize: 13, fontWeight: '700' }}>Γå╗ Refresh</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 15, paddingBottom: 80 }}>
              {visibleTasks.length === 0 ? (
                <View style={s.emptyState}>
                  <Text style={{ fontSize: 40 }}>Γ£à</Text>
          ...
          <ScrollView contentContainerStyle={{ padding: 15 }}>
            ...
            {loadingHistory ? (
              ...
              <View style={s.emptyState}>
                <Text style={{ fontSize: 40 }}>≡ƒôï</Text>
              ...
              history.map((h, i) => (
                <View key={i} style={s.historyCard}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    <Text style={s.historyMeta}>≡ƒôì {h.sector}</Text>
      ...
      <View style={s.bottomNav}>
        <TouchableOpacity style={s.navItem} onPress={() => handleTabChange('tasks')}>
          <Text style={[s.navIcon, tab === 'tasks' && { color: C.primary }]}>≡ƒôì</Text>
        ...
        <TouchableOpacity style={s.navItem} onPress={() => handleTabChange('history')}>
          <Text style={[s.navIcon, tab === 'history' && { color: C.primary }]}>≡ƒôï</Text>
        ...
        <TouchableOpacity style={s.navItem} onPress={() => handleTabChange('settings')}>
          <Text style={[s.navIcon, tab === 'settings' && { color: C.primary }]}>ΓÜÖ∩╕Å</Text>

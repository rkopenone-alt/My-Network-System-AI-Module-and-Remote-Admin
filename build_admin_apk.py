...
admin_dir = r"C:\Users\Alienware\Desktop\Rescue Backup AI 09-06-2026\admin-app"
...
import re
new_strings = re.sub(r'<string name="app_name">.*?</string>', '<string name="app_name">ARDMS Admin</string>', strings_content)
with open(strings_xml, 'w', encoding='utf-8') as f:
    f.write(new_strings)

new_gradle = re.sub(r"applicationId\s+['\"].*?['\"]", "applicationId 'com.rescue.admin'", gradle_content)
...
if result.returncode == 0:
    ...
    if os.path.exists(apk_path):
        ...
        shutil.copy(apk_path, os.path.join(output_dir, "Admin_App_Rescue.apk"))
        print("Admin APK successfully built and copied to Output_APKs.")
else:
    print("Build failed!")
